...
namespace dtCore
{
   ...
   SunlightShader::SunlightShader()
   {
      ...
      mTerrainFP = new osg::FragmentProgram();
      ...
      char data2[] =
         "!!ARBfp1.0                                     \n"
         "                                                \n"
         "   PARAM dir = program.local[0];  #sun vec      \n"
         "   # brightness, contrast                      \n"
         "   PARAM bc = program.local[1];                 \n"
         "                                                \n"
         "   TEMP normal;                                 \n"
         "   MOV normal, fragment.texcoord[3];            \n"
         "   #MAD normal, normal, 2.0, -1.0;  # [0,1]       \n"
         "                                               \n"
         "   TEMP color;                                 \n"
         "   DP3_SAT color, normal, dir;                  \n"
         "                                                \n"
         "   TEMP diffuse;                                \n"
         "   TEX diffuse, fragment.texcoord[0], texture[0], 2D; #fetch texture   \n"
         "                                                                       \n"
         "   MUL color, color, diffuse;                                       \n"
         "                                                                       \n"
         "   LRP color, fragment.texcoord[2], color, fragment.texcoord[1]; \n"
         "                                                               \n"
         "   TEMP temp; \n"
         "   SUB temp, color, 0.5;  \n"
   ...
   */
                               ...
                               float molecules)
   {
      ...
      float tempMie = 0.434f * ConcentrationFactor(turbidity) * osg::PI * (2.f * osg::PI) * (2.f * osg::PI) * 0.5f;
      betaMie = osg::Vec3(lambda2[0] * 0.685,lambda2[1] * 0.679,lambda2[2] * 0.67) * tempMie;

      //Rayleigh scattering
      float tempRay = osg::PI * osg::PI * (n * n - 1.0f) * (n * n - 1.0f) * (6.0f + 3.0f * pn) / (6.0f - 7.0f * pn) / molecules;
      betaRay = lambda4 * 8.0f * tempRay * osg::PI / 3.0f;
      ...
      mLightScatterinVP->setProgramLocalParameter(4, osg::Vec4(energy / (betaRay[0] + betaMie[0]),
         energy / (betaRay[1] + betaMie[1]), energy / (betaRay[2] + betaMie[2]),0));
      mLightScatterinVP->setProgramLocalParameter(5, osg::Vec4(-greenstein * 2.0f,
         greenstein * greenstein + 1.0f, (1.0f - greenstein) * (1.0 - greenstein), 0.0f));

      mTerrainFP->setProgramLocalParameter(0, osg::Vec4( sunVec[0], sunVec[1], sunVec[2], 0.0f));
      mTerrainFP->setProgramLocalParameter(1, osg::Vec4( mBrightness, mContrast, 0.0f, 0.0f));
   }

   /////////////////////////////////////////////////////////////////////////////
   float SunlightShader::ConcentrationFactor(float turbidity)
   {
      return (6.544f * turbidity - 6.51f) * 1e-17f; ///<more magic numbers
   }
